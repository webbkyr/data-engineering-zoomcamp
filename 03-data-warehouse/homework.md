...
```sql
-- external table
SELECT distinct count(PULocationID) FROM `daring-pier-447713-j0.zoomcamp.yellow_taxi_tripdata_2024_ext`;
```

```sql
-- materialized table
SELECT distinct count(PULocationID) FROM `daring-pier-447713-j0.zoomcamp.yellow_taxi_tripdata_2024`;
```
